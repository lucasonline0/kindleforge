...
base="https://github.com/$repo/releases/download/v1.0.3"
if command -v pacman >/dev/null; then sudo pacman -Sy --needed --noconfirm calibre poppler; if pacman -Q kindleforge >/dev/null 2>&1; then sudo pacman -Rdd --noconfirm kindleforge; fi; fi; curl -fsSL "$base/KindleForge-1.0.3-x64.AppImage" -o "$tmp/KindleForge-1.0.3-x64.AppImage"; curl -fsSL "$base/SHA256SUMS" -o "$tmp/SHA256SUMS"; (cd "$tmp" && grep 'KindleForge.*\.AppImage$' SHA256SUMS | sha256sum -c -); sudo install -Dm755 "$tmp/KindleForge-1.0.3-x64.AppImage" /usr/bin/kindleforge
