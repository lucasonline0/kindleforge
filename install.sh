...
if command -v pacman >/dev/null; then sudo pacman -Sy --needed --noconfirm calibre poppler; curl -fsSL "$base/kindleforge-1.0.0-x64.pacman" -o "$tmp/kindleforge.pkg.tar.zst"; curl -fsSL "$base/SHA256SUMS" -o "$tmp/SHA256SUMS"; (cd "$tmp" && sha256sum -c SHA256SUMS --ignore-missing); sudo pacman -U --noconfirm "$tmp/kindleforge.pkg.tar.zst"; else mkdir -p "$HOME/.local/bin"; curl -fsSL "$base/KindleForge-1.0.0-x64.AppImage" -o "$HOME/.local/bin/kindleforge"; chmod +x "$HOME/.local/bin/kindleforge"; fi
